...
function distanceInfo(user, placeInDivLocal){
...
function calldone(){
	...
	if(allCallbacks == totalCallbacks) {
		// Driver is taveling toMiles(totalDistance) miles. 
    		document.getElementById(placeInDivGlobal).innerHTML = driverInfo[0][0]+ " is traveling " + toMiles(totalDistance) + " miles. ";
		// Get drivers mpg
		var mpg = driverInfo[0][5];
		// Handel no mpg set
		if (mpg == null) {
			document.getElementById(placeInDivGlobal).innerHTML += "They have not given their mpg to calculate the cost per person. ";
			}
		// If there is a mpg
		else {
			...
			splitCost(totalRideCost,driverInfo[0][0], function(price){
				document.getElementById(placeInDivGlobal).innerHTML += "This will take "+ price +" dollars. ";
				});
			}
		}
	}
